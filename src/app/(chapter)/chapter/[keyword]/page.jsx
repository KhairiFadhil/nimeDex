...
async function Page({ params }) {
  ...
  const MangaData = responseManga?.relationships?.find(
    (relationship) => relationship.type === "manga"
  );
  const responseChapterFeed = await FetchMangaChapterFeed(
    MangaData.id,
    responseManga.attributes.translatedLanguage
  );
  const chapterFeed = responseChapterFeed?.data;

  //mencari chapter selanjutnya \
  const prevFilterChapter = chapterFeed.filter(
    (chapter) =>
      chapter?.attributes.chapter ==
      Number(responseManga?.attributes?.chapter) - 1
  );
  const nextFilterChapter = chapterFeed.filter(
    (chapter) =>
      chapter?.attributes.chapter ==
      Number(responseManga?.attributes?.chapter) + 1
  );
  //menentukan chapter dengan group yang sama jika tidak akan ke chapter yang ada
  const prevChapterGroup = prevFilterChapter.find(
    (chapter) =>
      chapter.relationships[0].id == responseManga.relationships[0].id
  );
  ...
  if (nextFilterChapter.length === 0) {
    ...
    const nextChapterGroup = nextFilterChapter.find(
      (chapter) =>
        chapter.relationships[0].id == responseManga.relationships[0].id
    );
    ...
    <div className="w-full h-full relative block">
      ...
      <div className="w-full h-full relative block z-10">
        <div className="w-full h-full max-w-[1440px] mx-auto bg-slate-200">
          <MangaPage api={response} chapterFeed={chapterFeed} />
        </div>
      </div>
      <div>
        <button
          hre
          className="w-full h-[70px] bg-red-700 hover:bg-red-600 active:bg-red-400 transition-all"
        >
          Next Chapter
        </button>
      </div>
